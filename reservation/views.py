                ...
                if date == 'Friday' or date == 'Saturday' or date == 'Sunday':
                    if Reservation.objects.filter(day=day).count() < 11:
                        if Reservation.objects.filter(day=day, time=time).count() < 1:
                            ...
                            messages.success(request, "Time already reserved!")
                    else:
                        messages.success(request, "The Selected Day Is Full!")
                else:
                    messages.success(request, "The Selected Date Is Incorrect")
            else:
                messages.success(request, "Date not in correct time period!")
        else:
...
def userPanel(request):
    ...
    reservations = Reservation.objects.filter(
        customer=customer).order_by('day', 'time')
...
def userCancel(request, id):
    ...
    if reservation.customer == request.user:
        ...
        messages.add_message(
            request, messages.ERROR, 'You can only delete our own bookings!')
    ...
    if request.method == 'POST':
        ...
        if table_service != None:
            if day <= maxDate and day >= minDate:
                if date == 'Friday' or date == 'Saturday' or date == 'Sunday':
                    if Reservation.objects.filter(day=day).count() < 11:
                        if Reservation.objects.filter(day=day, time=time).count() < 1 or userSelectedTime == time:
                            ...
                            messages.success(
                                request, "Reservation Edited, check UserPanel")
                            return redirect('home')
                        else:
                            messages.success(
                                request, "Time Has Been Reserved Before!")
                    else:
                        messages.success(request, "The Selected Day Is Full!")
                else:
                    messages.success(request, "The Selected Date Is Incorrect")
                ...
                messages.success(
                    request, "Selected Date Isn't In The Correct Time Period!")
        else:
            messages.success(request, "Please Select A Table Service!")
        return redirect('userPanel')
...
def checkEditTime(times, day, id):
    ...
    for k in times:
        if Reservation.objects.filter(
                day=day, time=k).count() < 1 or time == k:
            x.append(k)
    return x
